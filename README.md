...
|-----|---|-------|-----------|-----------|-------------|-----------|---------|
| Gate 1 | 0.5 | 4 | 9.0 | 0.125 | 14.49 | **−4.4%** | DP noise drowns the signal |
| Arm A | 0 | 4 | — | 0 | 2.21 | **+93.6%** | no-DP ceiling: recipe is sound |
| Arm B | 0.5 | 48 | 23.3 | 0.036 | 9.59 | **+72.2%** | batch averaging recovers DP utility |

The story in one line: **Gate 1 didn't fail because the model was too small — it
failed because batch=4 puts ~14× more noise per gradient element than signal.**
Raising the batch to 48 averages the noise down by √12 and recovers most of the
...
- **`dp_lora/accountant.py`** — a dependency-free Rényi-DP accountant for the
  *subsampled* Gaussian mechanism (Mironov–Talwar–Zhang 2019), plus
  ...
  Gate 1). Cross-validated against Opacus; **conservative by construction** — it
  never under-states ε.
...
```bash
pip install dp-lora-edge            # core
pip install "dp-lora-edge[opacus]"  # + Opacus cross-validation
```
...
print(noise_per_element(0.5, 0.5, batch_size=4))   # 0.125  (drowns it)
print(noise_per_element(0.5, 0.5, batch_size=48))  # 0.036  (survives)
```
...
that it **agrees with Opacus within ~10% in the practical DP regime and never
under-estimates ε**, and — the centerpiece — that the no-op bug is mechanically
impossible to reintroduce (`grad(lora_A) ≡ 0`, `grad(lora_B) > 0`, and the
trainer's sanity gate fires on a degenerate setup).

## Scope & honesty

Research code, not a compliance certificate. The accountant is a *conservative
upper bound* — see [SECURITY.md](SECURITY.md) and the report's "Accountant
fidelity" section before making any external privacy claim. No data ships here;
examples use synthetic text or public `wikitext`.
